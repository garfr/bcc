...
void
initErrors(const unsigned char *newBuffer, size_t newBufferSize,
...
void
queueError(char *message, size_t start, size_t end) {
  Error err;
  err.message = message;
  err.start = start;
  err.end = end;
 ...
 */
static Location
calculateLocation(size_t start, size_t end) {
  size_t line = 1;
  size_t column = 1;
  size_t i;
...
printError(Error *err) {
  ...
  printf(BOLDWHITE);
  printf("%s:%zd:%zd: ", errorCtx.bufferName, loc.line_start, loc.column_start);
  printf(BOLDRED);
  printf("error: ");
  printf(BOLDWHITE);
  printf("%s.\n", err->message);
...
bool
errorsExist() {
  return errorCtx.errors->numItems != 0;
}
