...
bool
compareSymbol(Symbol sym1, Symbol sym2) {
...
bool
compareSymbolStr(Symbol sym, const char *str) {
 ...
 */
HashEntry *
findOrInsertHashtbl(Hashtbl *tbl, Symbol sym, void *data) {
...
HashEntry *
insertHashtbl(Hashtbl *tbl, Symbol sym, void *data) {
  size_t hash = hashSymbol(sym) % tbl->numBuckets;
...
HashEntry *
findHashtbl(Hashtbl *tbl, Symbol sym) {
  size_t hash = hashSymbol(sym) % tbl->numBuckets;
...
Vector *
newVector(size_t itemSize, size_t initialSize) {
...
bool
pushVector(Vector *vec, void *data) {
...
void *
indexVector(Vector *vec, size_t index) {
...
HashEntry *
findInScope(Scope *scope, Symbol sym) {
